...
@Component(
	    property = {
	        ...
	        "mvc.command.name=/getLoginUserDetail"
	    },
	    service = MVCResourceCommand.class
	)
public class GetLogInUserDetailResourceCommand implements MVCResourceCommand{

	Log _log = LogFactoryUtil.getLog(GetLogInUserDetailResourceCommand.class.getName());
	...
	public boolean serveResource(ResourceRequest resourceRequest, ResourceResponse resourceResponse)
		...
		if(Validator.isNotNull(emailAddress)){
			try {
				...
				responseObj.put("userId", user.getUserId());
				CustomUser customUser = CustomUserLocalServiceUtil.getCustomUserByLRUserId(user.getUserId());
				if(Validator.isNotNull(customUser)){
					responseObj.put("mobNO", customUser.getMobileNo());
					responseObj.put("countryCode", customUser.getMobCountryCode());
					responseObj.put("deviceInfo", customUser.getDeviceInfo());

...
<div style="display: table-cell; padding-bottom: 50px;">
	<div class="main-container">
		<div class="col-sm-12"
			style="color: #961622; font-size: 14px; font-weight: bold;">
			You are almost there! We require additional information to complete
			your account setup</div>
		<div class="col-sm-4" style="margin-top: 20px; font-weight: bold;">
			Profile ID: ${userBean.partyBean.prtyid }</div>
		<div class="col-sm-4"
			style="color: #961622; font-size: 14px; margin-top: 20px; font-weight: bold; text-align: center;">
			${userBean.firstName } ${userBean.lastName }</div>
		<div class="col-sm-4" style="text-align: right; padding-right: 0px;">
			<%
				String ucntry = (String) renderRequest.getAttribute("userCountry");
			%>
			<img
				src="<%=themeDisplay.getPathThemeImages() + "/flag/" + ucntry + ".png"%>"
				height="50" width="50" />
		</div>

		<div class="col-sm-12"
			style="border: 3px solid #F2F2F2; border-left: 3px solid #11AB3E; border-radius: 5px; padding: 20px 10px; margin-top: 20px;">
			<table style="width: 100%;">
				<tr>
					...
					<td style="color: #308151; text-align: center; font-size: 12px;">
						...
						style="font-size: 28px; color: #000000;     margin-top: 0px;"></i><br /> Verified
					</td>
					<td style="color: #308151; text-align: center; font-size: 12px;">
						<i class="fa fa-envelope" aria-hidden="true"
						style="font-size: 24px; color: #000000;"></i><br /> Verified
					</td>
		...
		<c:choose>
			...
			<c:otherwise>
				<div class="col-sm-12"
					style="border: 3px solid #F2F2F2; border-left: 3px solid #EE0D16; border-radius: 5px; padding: 20px 10px; margin-top: 20px;">
			</c:otherwise>
		</c:choose>

		<aui:form name="uploadDocFm" action="${uploadDocumentURL}"
						...
						<c:choose>
							...
							<c:otherwise>
								<i class="fa fa-exclamation-triangle" aria-hidden="true"
									style="font-size: 20px; color: #FFCC00;"></i>
							</c:otherwise>
						</c:choose>
			...
			<div id="upload1-expand2"
				style="margin-left: 30px; margin-top: 30px;">

				<table>
					<tr>
						<td class="padding-left-10"><input type="file"
							id="verificationDoc" name="verificationDoc" required="required"
  ...
  <c:if test="${!accountVerified && !accountRemindLater}">
	<c:choose>
		...
		<aui:form name="accountFm" action="${updateAccountURL}"
			cssClass="form-horizontal form-label-left">
			<table style="width: 100%;">
				<tr>
					<td style="width: 25px;">
					...
					<td style="padding-left: 10px;"><span
						style="font-size: 14px; font-weight: bold;">Set up a
							funding account to send money (Debit card or bank account
							accepted)</span><br />
						<div style="margin-top: 15px;">
							<aui:input id="debit_card" type="radio" name="accountType"
								value="debit_card" label="" />
							<label for="<portlet:namespace/>debit_card" checked="true"
								style="color: black;">Debit Card</label>
							<aui:input id="bank_account" type="radio" name="accountType"
								value="bank" label="" style="margin-left: 25px;" />

							<label for="<portlet:namespace/>bank_account">Bank
								Account</label>
						</div></td>
					<td style="width: 50px; color: #308151; text-align: center;">
						<c:choose>
			...
			<div id="debit_card_detail"
				...
				<table>
					...
					<tr>
						<td><aui:input name="cardNumber" label=""
								placeholder="Card Number"
								cssClass="form-control col-md-7 col-xs-6">
								<aui:validator name="number" />
								<aui:validator name="maxLength">20</aui:validator>
							</aui:input></td>
						<td class="padding-left-20"><span id="visa"
							style="display: none;"><img
								src="<%=themeDisplay.getPathThemeImages() + "/visa.jpg"%>"
								width="40" height="25" style="margin-bottom: 14px;" /></span> <span
							id="mastercard" style="display: none;"><img
								src="<%=themeDisplay.getPathThemeImages() + "/mastercard.jpg"%>"
								width="40" height="25" style="margin-bottom: 14px;" /></span></td>
					</tr>
					<tr style="height: 35px; vertical-align: bottom;">
						<td class="">First name on account</td>
						<td style="padding-left: 26px;">Last name on Account</td>
					</tr>
					<tr>
						<td style="vertical-align: initial;"><aui:input
								name="cardFirstName" label="" placeholder="First Name"
								cssClass="form-control col-md-7 col-xs-6"
								value="${ userBean.partyBean.accountBean.fName}">
								<aui:validator name="maxLength">50</aui:validator>
							</aui:input></td>
						<td
							style="padding-left: 26px; display: inline-block; vertical-align: initial;">

							<aui:input name="cardLastNumber" label="" placeholder="Last Name"
								cssClass="form-control col-md-7 col-xs-6"
								value="${ userBean.partyBean.accountBean.lName}"
								style="; width: 100%; border:1px solid #D3D3D3; border-radius: 1px;">
								<aui:validator name="maxLength">50</aui:validator>
							</aui:input>
					...
					<tr>
						<td class="padding-top-10">Expires on</td>
						<td class="padding-left-20 padding-top-10"></td>
					</tr>
					<tr>
						<td colspan="2">
							<div class="col-sm-12 pad-0">
								<div class="col-sm-3 pad-0">
									<aui:select name="expireOnMonth" label=""
										...
										<aui:option value="00"
											selected="${userBean.partyBean.accountBean.exprymnth eq '00' ? true : false }">01</aui:option>
										<aui:option value="01"
											selected="${userBean.partyBean.accountBean.exprymnth eq '01' ? true : false }">02</aui:option>
										<aui:option value="02"
											selected="${userBean.partyBean.accountBean.exprymnth eq '02' ? true : false }">03</aui:option>
										<aui:option value="03"
											selected="${userBean.partyBean.accountBean.exprymnth eq '03' ? true : false }">04</aui:option>
										<aui:option value="04"
											selected="${userBean.partyBean.accountBean.exprymnth eq '04' ? true : false }">05</aui:option>
										<aui:option value="05"
											selected="${userBean.partyBean.accountBean.exprymnth eq '05' ? true : false }">06</aui:option>
										<aui:option value="06"
											selected="${userBean.partyBean.accountBean.exprymnth eq '06' ? true : false }">07</aui:option>
										<aui:option value="07"
											selected="${userBean.partyBean.accountBean.exprymnth eq '07' ? true : false }">08</aui:option>
										<aui:option value="08"
											selected="${userBean.partyBean.accountBean.exprymnth eq '08' ? true : false }">09</aui:option>
										<aui:option value="09"
											selected="${userBean.partyBean.accountBean.exprymnth eq '09' ? true : false }">10</aui:option>
										<aui:option value="10"
											selected="${userBean.partyBean.accountBean.exprymnth eq '10' ? true : false }">11</aui:option>
										<aui:option value="11"
											selected="${userBean.partyBean.accountBean.exprymnth eq '11' ? true : false }">12</aui:option>
									</aui:select>
								</div>
								<div class="col-sm-3" style="padding: 0; margin-left: 20px;">
									<aui:select name="expireOnYear" label=""
										style="font-size: 12px;     max-width: 100px;">
										<aui:option value="">YYYY</aui:option>
										<aui:option value="18"
											selected="${userBean.partyBean.accountBean.expryyear eq '18' ? true : false }">2018</aui:option>
										<aui:option value="19"
											selected="${userBean.partyBean.accountBean.expryyear eq '19' ? true : false }">2019</aui:option>
										<aui:option value="20"
											selected="${userBean.partyBean.accountBean.expryyear eq '20' ? true : false }">2020</aui:option>
										<aui:option value="21"
											selected="${userBean.partyBean.accountBean.expryyear eq '21' ? true : false }">2021</aui:option>
										<aui:option value="22"
											selected="${userBean.partyBean.accountBean.expryyear eq '22' ? true : false }">2022</aui:option>
										<aui:option value="23"
											selected="${userBean.partyBean.accountBean.expryyear eq '23' ? true : false }">2023</aui:option>
										<aui:option value="24"
											selected="${userBean.partyBean.accountBean.expryyear eq '24' ? true : false }">2024</aui:option>
										<aui:option value="25"
											selected="${userBean.partyBean.accountBean.expryyear eq '25' ? true : false }">2025</aui:option>
										<aui:option value="26"
											selected="${userBean.partyBean.accountBean.expryyear eq '26' ? true : false }">2026</aui:option>
										<aui:option value="27"
											selected="${userBean.partyBean.accountBean.expryyear eq '27' ? true : false }">2027</aui:option>
										<aui:option value="28"
											selected="${userBean.partyBean.accountBean.expryyear eq '28' ? true : false }">2028</aui:option>
									</aui:select>
								</div>
							</div>
						</td>
						<td class="padding-left-20"></td>
					</tr>
					<tr>
			...
			<div id="bank_detail"
				style="display: none; margin-left: 30px; margin-top: 30px;">
				<table>
					<tr>
						<td><aui:input name="accountNumber" label="Account Number"
								placeholder="Account Number"
								cssClass="form-control col-md-7 col-xs-6"
								style="font-size: 12px; ; width: 100%; border:1px solid #D3D3D3; border-radius: 1px;"
								>
								<aui:validator name="required" />
							</aui:input></td>
						<td style="padding-left: 52px;"><aui:input
								name="accountNumber2" label="Re Enter Account Number"
					...
					<tr>
						<td class="padding-top-10"><aui:input id="known_branch"
								type="radio" name="searchBranchType" value="known_branch"
								label="" /> <label for="<portlet:namespace/>known_branch"><span
								id="unique_branch_code_label">Routing No.</span></label> <aui:input
								id="search" type="radio" name="searchBranchType" value="search"
								label="" style="margin-left: 25px;" /> <label
							for="<portlet:namespace/>search">Search</label></td>
						<td class="padding-left-20 padding-top-10"></td>
					</tr>
					<tr>
						<td colspan="2">
							<div class="col-sm-4 pad-0" id="known_branch_detail">
								<aui:input type="text" name="unique_branch_code"
									label="Branch Code"></aui:input>
							</div>
							<div class="col-sm-12 pad-0" id="search_branch_detail">
								<div class="col-sm-4 city-auto pad-side-10"
									style="padding-left: 0;">
									<aui:input name="countryCity" label="City"
										style="font-size: 12px; ;"
										value="${userBean.partyBean.accountBean.branchBean.cityName}" />
	...
	AUI()
			.use(
					...
					function(A) {
						...
						var accountTypeBtns = A
								.all("input[name=<portlet:namespace />accountType]");
						var searchBranchTypeBtns = A
								.all("input[name=<portlet:namespace />searchBranchType]");
						var cardNumber = A
								.one("#<portlet:namespace />cardNumber");
						var bankSelect = A
								.one("#<portlet:namespace />bankName");
						var stateSelect = A
								.one("#<portlet:namespace />state");
						var citySelect = A
								.one("#<portlet:namespace />countryCity");
								...
								.on(
										...
										function(e) {
											var formValidator = Liferay.Form
													.get('<portlet:namespace />uploadDocFm').formValidator;
											formValidator.validate();
											if (!formValidator.hasErrors()) {
												document.<portlet:namespace />uploadDocFm
														.submit();
											}
										});
					  }
						...
						updateAccountCardBtn
								.on(
										...
										function(e) {
											console
													.log("updateAccountCardBtn -> click");
											if (A
													.one("#<portlet:namespace />debit_card")._node.checked) {
												console
														.log("debitCard validator");
												debitCardValidator.validate();
												console
														.log("debitCardValidator.hasErrors()->"
																+ debitCardValidator
																		.hasErrors());
												if (!debitCardValidator
														.hasErrors()) {
													document.<portlet:namespace />accountFm
															.submit();
												}
											}
										});
						...
						updateAccountBankBtn
								.on(
										...
										function(e) {
											...
											if (A
													.one("#<portlet:namespace />bank_account")._node.checked) {

												if (A
														.one("#<portlet:namespace />search")._node.checked) {
													...
													knownBranhcValidator
															.validate();
													console
															.log("knownBranhcValidator"
																	+ knownBranhcValidator
																			.hasErrors());
													if (!knownBranhcValidator
															.hasErrors()) {
														document.<portlet:namespace />accountFm
																.submit();
													}
						...
						var cityAutoComplete = new A.AutoCompleteList(
								{
									...
									source : function() {
										...
										var finalData;
										var cityAutoCompelteRequest = A.io
												.request(
														getCityDetailURL
																.toString(),
														{
															...
															data : {
																...
																<portlet:namespace />stateCode: '${userBean.partyBean.partyAddressBean.state}',
																<portlet:namespace />countryCode: '${userBean.partyBean.partyAddressBean.cntrycd}'
															},
															autoLoad : false,
															sync : false,
															on : {
						...
						function simulateBanksOnCityChange(cityId, countryCode) {
							...
							console.log("simulateBanksOnCityChange->" + cityId
									+ "," + countryCode);

							var getBankDetailURL = '${getBankDetailURL}';
							A.io
									.request(
											getBankDetailURL.toString(),
											{
												dataType : 'json',
												method : 'GET',
												data : {
													...
													success : function() {
														...
														for (var i = 0; i < bankList.length; i++) {
															if (selectedBankId != '') {
																...
																console
																		.log("comparing bank ->"
																				+ selectedBankId == (bankList[i].bankId));
															...
															A
																	.one(
																			'#<portlet:namespace/>bankName')
																	.append(
																			'<option value="'+ bankList[i].bankId +','+ bankList[i].bankNm +'">'
																					+ bankList[i].bankNm
																					+ '</option>');
														}
																		...
																		for ( var i in branchList) {
																			...
																			A
																					.one(
																							'#<portlet:namespace/>branchName')
																					.append(
																							'<option value="'+ branchList[i].branchId +','+ branchList[i].branchRtngnr +'">'
																									+ branchList[i].branchDes
																									+ '</option>');
																		}
						...
						if (accountType==1) {
							if (parseInt(citySelected) > 0) {
								...
								simulateBanksOnCityChange(citySelected,
										countryCode);
							}
						}
						if (accountType==2) {
